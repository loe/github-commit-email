...
class Commit < Merb::Controller
  ...
  def create
    ...
    ch['commits'].each do |commit|
      first_line_of_commit_message = commit['message'].split('\n').first
      subject = "#{commit['author']['name']} committed to #{ch['repository']['name']}: #{first_line_of_commit_message}"
      body = <<-EOH
#{commit['url']}
...
EOH
      ...
      begin
        diff.close
        system("cd /tmp/#{ch['repository']['name']} && git show #{commit['id']} > #{diff.path}")
      ...
      m.deliver!
      Merb.logger.info("#{Time.now}: #{commit['id']} Sent")
    end
  end
end